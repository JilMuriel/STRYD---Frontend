...
const MetricCard = ({ data }: MetricCardProps) => {
  ...
  return (
    <div className="bg-surface-container-lowest rounded-xl p-6 shadow-[0_20px_40px_rgba(23,29,28,0.03)] flex flex-col relative overflow-hidden group hover:-translate-y-1 transition-transform duration-300">
      <div className="flex justify-between items-start mb-4 z-10">
        ...
        <span className={`inline-flex items-center px-2 py-1 rounded-full font-medium tex890t-label-caps ${visualState.badgeClassName}`}>
          {visualState.badgeText}
        </span>
      </div>
      <div className="mt-auto z-10">
        <span className="font-data-point text-data-point font-bold mb-2.5">{formatMetricValue(data.value)}</span>
